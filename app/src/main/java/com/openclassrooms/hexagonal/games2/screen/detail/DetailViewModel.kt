package com.openclassrooms.hexagonal.games2.screen.detail
...
import com.openclassrooms.hexagonal.games2.domain.model.Comment
    ...
    private suspend fun getCommentsByPostId(postId: String): List<Comment> {
        return try {
            val commentsSnapshot = firestore.collection("comments")
                .whereEqualTo("post.postId", postId)
                .orderBy("timestamp", Query.Direction.DESCENDING)
                .get()
                .await()

            Log.d("getCommentsByPostId", "Snapshot size: ${commentsSnapshot.size()}")

            // Log each document in the snapshot
            commentsSnapshot.documents.forEach { document ->
                Log.d("getCommentsByPostId", "Document: ${document.data}")
            }
